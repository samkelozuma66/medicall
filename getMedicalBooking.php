<?php
    ...
    {
        if ($stmt = $conn->prepare('SELECT patient_name, patient_surname, patient_number, patient_address, booking_status, MedicalAid_Name, MedicalAid_Number, _date, _time, booking_number FROM medic_bookings WHERE user_id = ?')) 
            {
            	...
            	$stmt->store_result();
            	$stmt->bind_result($patient_name,$patient_surname,$patient_number,$patient_address,$booking_status,$MedicalAid_Name,$MedicalAid_Number,$_date,$_time,$booking_number);
            	$json_array = array();
